...
A Zellij plugin that aggregates Pi processes in the current Zellij session and shows their project/workspace, Plan-mode state, context usage, goals, todos, progress, active tools, agents, elapsed/busy time, and token usage.
...
type Status = {
  ...
  sessionName?: string; instanceName?: string; cwd?: string; workspace?: Workspace;
  mode?: "normal" | "plan"; model?: string; thinking?: string;
